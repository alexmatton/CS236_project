...
class SemiLocalGenerativeTransformer(ConditionedGenerativeModel):
    ...
    def __init__(
        ...
        super(SemiLocalGenerativeTransformer, self).__init__(embd_size)
        self.model_type = 'Transformer'

        # Model:
        self.pos_encoder = PositionalEncoding(d_model, dropout)
        encoder_layers = TransformerEncoderLayer(d_model, nhead, d_model * 4, dropout)
        self.transformer_encoder = TransformerEncoder(encoder_layers, 1) # one layer only
        self.d_model = d_model
        self.nlayers = nlayers
        self.color_to_embedding = nn.Linear(3, d_model)
        self.position_local_t = position_local_t
        self.kz = kz
        ...
        self.mask_local, self.mask_nonlocal = None, None
        self.h, self.w = None, None

        print("Type of semilocal transformer:", self.position_local_t)
        print("Kernel size", self.kz)
    ...
    def _generate_local_mask(self, sz, kz):
        """ Returns tensor (sz * sz) with 0 in the kernel, and -infty otherwise """
        mask = torch.zeros(sz, sz)
        h = int(math.sqrt(sz))
        for current_pos in range(sz):
            ...
            for other_pixel_pos in range(current_pos+1):
                ...
                if abs(current_h - other_pixel_h) + abs(current_w - other_pixel_w) <= kz:
                    mask[current_pos, other_pixel_pos] = 1
        mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
        return mask
    ...
    def forward(self, imgs, condition_embd):
        ...
        if self.mask_local is None:
            self.mask_nonlocal = self._generate_square_subsequent_mask(imgs.size(0)).to(device) #h, w
            self.mask_local = self._generate_local_mask(imgs.size(0), self.kz).to(device) #h, w
        ...
        current_embeddings = imgs_for_prediction # h*w, bsize, d_model

        if self.position_local_t == "in_between":
            for _ in range(self.nlayers//2):
                current_embeddings = self.transformer_encoder(current_embeddings, self.mask_local)
                current_embeddings = self.transformer_encoder(current_embeddings, self.mask_nonlocal)
        elif self.position_local_t == "local_before":
            for _ in range(self.nlayers//2):
                current_embeddings = self.transformer_encoder(current_embeddings, self.mask_local)
            for _ in range(self.nlayers//2):
                current_embeddings = self.transformer_encoder(current_embeddings, self.mask_nonlocal)
        ...
        output_transformer = current_embeddings.reshape(h, w, bsize, self.d_model)
...
if __name__ == "__main__":
    ...
    transformer_model = SemiLocalGenerativeTransformer(16, 4, 2, 4, position_local_t='local_before') #embedzise, d_model, nhead, nlayers
    transformer_model.eval()

    # print(transformer_model._generate_local_mask(5*5,2))
